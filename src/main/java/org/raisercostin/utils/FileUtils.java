...
public class FileUtils {
  ...
  public static final void cleanDirectory(File directory) throws IOException {
    if (log.isDebugEnabled()) {
      log.debug("Clean folder [" + directory.getAbsolutePath() + "].");
    }
    if (directory.exists()) {
  ...
  public static File forceMkdir(File directory) throws IOException {
    if (log.isDebugEnabled()) {
      log.debug("Create folder [" + directory.getAbsolutePath() + "] if doesn't exists.");
    }
  ...
  public static void moveContent(Map<File, File> movedFiles) {
    for (Map.Entry<File, File> entryFile : movedFiles.entrySet()) {
      try {
        ...
        log.warn("Couldn't rename from [" + entryFile.getKey().getAbsolutePath() + "] to ["
            + entryFile.getValue().getAbsolutePath() + "].",
          e);
      }
    }
  }
  ...
  public static void cleanup(List<File> result) {
    for (File file : result) {
      try {
        ...
        log.warn("Couldn't remove [" + file.getAbsolutePath() + "].", e);
      }
    }
  }
    ...
    try {
      ...
      if (in != null) {
        try {
          ...
          log.warn("Couldn't close a stream on url " + urlRes + " with encoding " + encoding + ".", exc);
        }
      }
    }
