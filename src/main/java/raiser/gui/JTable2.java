...
/**
 * A JTable with a few bugfixes.
 * This class is the class FTable taken from http://www.chka.de/swing/table/ .
 * @since 2004.12.15
 */
public class JTable2 extends JTable
{
    ...
    public boolean getScrollableTracksViewportWidth()
    {
        return getAutoResizeMode() != AUTO_RESIZE_OFF
                || (getParent() instanceof JViewport && getParent().getWidth() > getPreferredSize().width);
    }
    ...
    public boolean editCellAt(int row, int column, EventObject e)
    {
        ...
        if (row < 0 || row >= getRowCount() || column < 0
                || column >= getColumnCount())
            throw new IllegalArgumentException("row="+row+" col="+column+" e="+e);
