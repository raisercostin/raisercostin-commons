...
public class Formatter {
    ...
    public static Calendar parseDate(String date) {
        for ( Map.Entry<String,Formatter.FormatterEntry> entry : parsers.entrySet()) {
            try {
                ...
                logger.info("Can't parse with "+entry.getValue().getName()+"[" + entry.getValue().getValue() + "] the date ["
                        + date + "].");
            }
        }
        return null;
    ...
    static{
        ...
        for (int index = 0; index < DATE_FORMAT.length; index++) {
            addParser(DATE_FORMAT[index][0], DATE_FORMAT[index][1]);
        }
    }
