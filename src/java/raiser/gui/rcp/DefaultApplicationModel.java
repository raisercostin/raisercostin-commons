...
public abstract class DefaultApplicationModel implements ApplicationModel {
     ...
     */
    private static final Logger logger = Logger.getLogger(DefaultApplicationModel.class);
    ...
    public String getVersionHTML() {
        return "<html>" + "Project: " + getApplicationUniqueName() + "<br>\n" + "Version: "
                + getApplicationVersion() + "<br>\n" + "Build: " + getApplicationBuild() + "<br>\n"
                + "BuildDate: " + getApplicationBuildDate() + "<br>\n" + "BuildType: "
                + getApplicationBuildType() + "<br>\n"
    ...
    public void loadProperties() throws IOException, ClassNotFoundException {
        ...
        try {
            ...
            logger.info("Found configure.properties file: "+new File("configure.properties").getAbsolutePath());
        } catch (FileNotFoundException e) {
            is = getClass().getClassLoader().getResourceAsStream("configure.properties");
...
//        setOutputFormat(getProperty("output.format", "{0}{1} - {2} - {3} - {4} - {5}.{8}"));
//        setKeepOldFile(getProperty("keepOldFile", "true").equals("true"));
//        setSaveId3v1(search(getProperty("output.saveID3v1Version", TagFactory.ID3V_1_NONE), getId3v1Options()));
//        setSaveId3v2(search(getProperty("output.saveID3v2Version", TagFactory.ID3V_2_4_0), getId3v2Options()));
    ...
    public Object getProperty(String key, Object defaultValue) {
        ...
        if (result == null) {
            logger.info("Can't find a value for property [" + key + "]. Using the default value ["
                    + defaultValue + "]");
            result = defaultValue;
        }
        return result;
