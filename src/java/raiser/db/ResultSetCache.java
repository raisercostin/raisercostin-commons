...
public class ResultSetCache
{
	...
	public synchronized static void remove(String sid, String rsname)
	{
		...
		if (h != null)
		{
			//System.out.println("ResultSet " + rsname + " removed from session " + sid);
			
			h.remove(rsname);
			/*if (h.remove(rsname) != null)
				SimpleLogger.log(rsname + " removed ok from Database cache");
			else
				SimpleLogger.log(rsname + " remove failure from Database cache");*/
		}		
	}
	...
	public synchronized static void set(String sid, ResultSet rs)
	{
		...
		//System.out.println("ResultSet " + rs.toString() + " added in session " + sid);
